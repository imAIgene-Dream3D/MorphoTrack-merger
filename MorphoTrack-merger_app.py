...
# Function for morphological feature extraction (2D) - TrackMate
def morphological_features_2d_trackmate(seg_path, track_path):
...
# Function for morphological feature extraction (3D) - TrackMate
def morphological_features_3d_trackmate(seg_path, track_path):
    labels_4d = io.imread(seg_path)
    features = []
...
# Function for manual tracking (2D only)
def morphological_features_2d_manual(seg_path, track_path):
    labels_4d = io.imread(seg_path)
    features = []
    ...
    # Load TrackMate data
    trackmate_df = pd.read_csv(track_path, skiprows=lambda x: x in [1,2,3], encoding='ISO-8859-1')
    trackmate_coords = trackmate_df[['X', 'Y']].values
    tree = cKDTree(trackmate_coords)
    
    morph_coords = features_df[['Centroid_X', 'Centroid_Y']].values
    distances, indices = tree.query(morph_coords)

    features_df['Distance_to_ManualTracking'] = distances
    matched_trackmate_data = trackmate_df.iloc[indices].reset_index(drop=True)
    merged_df = pd.concat([features_df.reset_index(drop=True), matched_trackmate_data], axis=1)
    merged_df = merged_df[merged_df['Track n°'].notna()].drop_duplicates()

    return merged_df

# Function to plot the tracks
def plot_tracks_manual(df, mode):
    fig, ax = plt.subplots(figsize=(8, 6)) if mode == "2D" else plt.figure(figsize=(10, 8)).add_subplot(projection='3d')

    for track_id, track in df.groupby("Track n°"):
        if mode == "2D":
...
def plot_tracks_trackmate(df, mode):
    fig, ax = plt.subplots(figsize=(8, 6)) if mode == "2D" else plt.figure(figsize=(10, 8)).add_subplot(projection='3d')
...
tracking_method = st.radio("Select Tracking Method:", ["TrackMate", "Manual Tracking (Only 2D)"])

# Toggle for 2D/3D mode (only available for TrackMate)
mode = "2D" if tracking_method == "Manual Tracking (Only 2D)" else st.segmented_control("Choose Processing Mode:", ["2D", "3D"])

# File upload
seg_file = st.file_uploader("Upload Segmentation TIFF", type=['tif'], key="seg_upload")
track_file = st.file_uploader("Upload Tracking CSV (spots)", type=['csv'], key="track_upload")
...
if process_button and seg_file and track_file:
    with st.spinner("Processing..."):
        # Save uploaded segmentation file
        ...
        if tracking_method == "TrackMate":
            if mode == "2D":
                merged_df = morphological_features_2d_trackmate(seg_path, track_path)
            else:
                merged_df = morphological_features_3d_trackmate(seg_path, track_path)
        else:
            merged_df = morphological_features_2d_manual(seg_path, track_path)
        ...
        if tracking_method == "Manual Tracking (Only 2D)":
            plot_tracks_manual(merged_df, mode)
        else:
            plot_tracks_trackmate(merged_df, mode)
elif process_button:
    st.error("Please upload both Segmentation and Tracking files before processing.")
